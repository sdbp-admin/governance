...
export function RedesignAuthenticatedLaunch() {
  ...
  return <>{profile && <RedesignPrototype liveProfile={profile} accountControls={{ message: message || "Account", canManageAccess, onPeopleAccess: () => void openAccessManager(), onPassword: () => { setMessage(""); setError(""); setPasswordEditor(true); }, onSignOut: () => void signOut() }} />}{profile && accessOpen && <AccessManager people={accessPeople} currentProfileId={profile.id} presidentId={accessPresidentId} canTransferPresidency={canTransferPresidency} transferTargetId={transferTargetId} setTransferTargetId={setTransferTargetId} loading={accessLoading} busyId={accessBusyId} error={accessError} onTransfer={transferPresident} onResend={resendInvitation} onRemove={removeAccess} onClose={() => setAccessOpen(false)} />}</>;
}
