...
export function Header({ view, attentionCount, currentUserId }: { view: View; attentionCount: number; currentUserId: string }) {
  const descriptions: Record<View, string> = {
    ...
    work: "Keep outcomes visible. A resolved tension can still leave an open action or project behind.",
    ...
    governance: "Governance changes SDBP's ongoing roles, accountabilities, domains and policies. Start it from a tension that needs structural change.",
...
export function WorkView({ projects, actions, tensions, currentUserId, onCompleteAction }: { projects: Project[]; actions: Action[]; tensions: Tension[]; currentUserId: string; onCompleteAction: (actionId: string) => void }) {
  const openActions = actions.filter((action) => action.status !== "done" && action.status !== "cancelled");
  return <div className="work-layout"><section className="work-main"><div className="section-head"><div><span className="section-kicker">Current outcomes</span><h2>Active projects</h2></div></div><div className="project-grid">{projects.filter((project) => project.status === "active").map((project, index) => <article className={`project-card ${index === 0 ? "project-featured" : ""}`} key={project.id}><div className="project-accent" aria-hidden="true" /><span className="kind">{project.role ?? "SDBP project"}</span><h3>{project.title}</h3><p>{project.summary}</p><div className="project-meta"><span><strong>{personName(project.ownerId)}</strong><small>owner</small></span><span><strong>{formatShortDate(project.lastUpdate)}</strong><small>last updated</small></span><span><strong>{formatShortDate(project.nextPrompt)}</strong><small>next prompt</small></span></div></article>)}</div></section><aside className="action-rail"><div className="section-head"><div><span className="section-kicker">Concrete next steps</span><h2>Actions</h2></div></div><div className="action-stack">{openActions.length ? openActions.map((action) => {
    const sourceTension = tensions.find((tension) => tension.id === action.sourceTensionId);
    const sourceResolved = sourceTension?.status === "resolved";
    return <article className="action-slip" key={action.id}><span className="action-status">{action.status}</span><h3>{action.title}</h3><p>{action.source}</p>{sourceResolved && <small className="action-context">Source tension resolved. This commitment stays here until the action itself is completed.</small>}<div className="action-owner"><span className="mini-avatar">{personInitial(action.ownerId)}</span>{personName(action.ownerId)}</div>{action.status === "open" && action.ownerId === currentUserId && <button className="secondary small action-done" onClick={() => onCompleteAction(action.id)}>Mark done</button>}</article>;
