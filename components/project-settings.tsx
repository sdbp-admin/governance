...
export function ProjectSettings() {
  ...
  useEffect(() => {
    function findCards() {
      const next = Array.from(document.querySelectorAll(".project-grid .project-card"));
      setCards((current) => sameElements(current, next) ? current : next);
    }
    findCards();
  ...
  useEffect(() => {
    ...
    setParticipants([...new Set([selected.ownerId, ...(selected.participantIds ?? [])])]);
  ...
  const bindings = cards.map((card, index) => {
    const visibleTitle = card.querySelector("h3")?.textContent?.trim();
    const project = activeProjects[index] && activeProjects[index].title === visibleTitle
      ? activeProjects[index]
      : activeProjects.find((candidate) => candidate.title === visibleTitle);
    return project ? { card, project } : null;
  }).filter((binding): binding is { card: Element; project: Project } => Boolean(binding));

  return <>
    {bindings.map(({ card, project }) => createPortal(
      <div className="actions compact-actions" key={project.id}><button className="quiet small" type="button" onClick={() => openProject(project)}>Edit project</button></div>,
      card,
    ...
    {open && <div className="modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className={`workflow-editor compact-modal ${styles.modal}`} role="dialog" aria-modal="true" aria-label="Edit project">
        <div className="editor-head"><div><span className="section-kicker">Project settings</span><h2>{selected ? `Edit ${selected.title}` : "Edit project"}</h2></div><button className="quiet editor-close" type="button" onClick={() => setOpen(false)}>×</button></div>
        ...
        {loading && !selected ? <div className={styles.loading}>Loading project…</div> : selected ? <>
          <label className="field"><span>Project name</span><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} /></label>
          ...
          <div className="editor-actions"><div /><div className="editor-actions-right"><button className="secondary" type="button" onClick={() => setOpen(false)}>Cancel</button><button className="primary" type="button" disabled={saving || !title.trim() || !ownerId} onClick={() => void save()}>{saving ? "Saving…" : "Save project"}</button></div></div>
        </> : <div className={styles.empty}>This project is no longer active.</div>}
      </section>
    </div>}
  </>;
}
