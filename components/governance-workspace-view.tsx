...
export function GovernanceWorkspaceView({ workspace, currentUserId, personName, onCreateProposal, onStartMeeting, onGoTensions, onGoRecords }: {
  ...
  return <>
    ...
    {ready.length > 0 && <section className="section"><div className="section-head"><div><span className="section-kicker">Needs a proposal</span><h2>Structural tensions</h2></div></div><div className="governance-ready-list">{ready.map((tension) => <ProposalStarter key={tension.id} tension={tension} mine={tension.raiserId === currentUserId} personName={personName} workspace={workspace} onCreate={onCreateProposal} />)}</div></section>}
...
function QuickConsentPanel({ proposal, people, currentUserId, personName, onStartMeeting }: {
  ...
  if (supported === false) {
    return <div className="process-actions"><button className="primary" onClick={() => void onStartMeeting(proposal)}>Start governance meeting</button></div>;
  }
  ...
  if (!round) {
    return <div className="governance-round">
      ...
      <h4>Quick consent or governance meeting?</h4>
      <p>Use quick consent when the proposal is clear enough to process asynchronously. Everyone must explicitly record no objection. A single objection sends it to a governance meeting.</p>
      <div className="process-actions">
        {proposal.proposerId === currentUserId && <button className="secondary" type="button" disabled={busy || supported === null} onClick={() => void startConsent()}>{busy ? "Starting…" : "Quick consent"}</button>}
        <button className="primary" type="button" onClick={() => void onStartMeeting(proposal)}>Start governance meeting</button>
      </div>
  ...
  return <div className="governance-round">
    ...
    {objectionOpen && <div className="governance-inline-form">
      ...
      <div className="process-actions"><button className="quiet" type="button" onClick={() => { setObjectionOpen(false); setObjectionText(""); }}>Cancel</button><button className="primary" type="button" disabled={!objectionText.trim() || busy} onClick={() => void respond("objection")}>Raise objection</button></div>
    </div>}
    {error && <div className="auth-message error">{error}</div>}
  </div>;
}
